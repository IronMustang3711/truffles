...
/**
 * Checks whether it is the start of the normal frame, CC frame, or the data is
 * out of sync
 */
bool Pixy::getStart() {
  uint16_t w, lastw;
  ...
  while (true) {
    ...
    if (w == 0 && lastw == 0) {
      ...
      // When byte received was 0x55aa instead of other way around, the code
      // syncs the byte
      printf("Pixy: reorder");
      getByte();  // Resync
    }
  ...
  for (blockCount = 0;
    ...
    for (i = 0, sum = 0; i < sizeof(Block) / sizeof(uint16_t); i++) {
      if (blockType == NORMAL_BLOCK && i >= 5) {
        // If it's not a color-code block, there's no need to consider angle, so
        // skip it
        block->angle = 0;
        break;
      }
