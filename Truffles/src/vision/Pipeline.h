...
class Pipeline {
	private:
		Mat source0;
		Size newSizeOutput;
		Mat cvGaussianblurOutput;
		Mat cvCannyOutput;
		vector<Line> findLinesOutput;
		vector<Line> filterLinesOutput;
		void newSize(double , double , Size &);
		void cvGaussianblur(Mat &, Size &, double , double , int , Mat &);
		void cvCanny(Mat &, double , double , double , bool , Mat &);
		void findLines(Mat &, vector<Line> &);
		void filterLines(vector<Line> &, double , double [], vector<Line> &);

	public:
		Pipeline();
		void Process();
		void setsource0(Mat &source0);
		Size* getnewSizeOutput();
		Mat* getcvGaussianblurOutput();
		Mat* getcvCannyOutput();
		vector<Line>* getfindLinesOutput();
		vector<Line>* getfilterLinesOutput();
};
