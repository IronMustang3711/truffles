...
RotateCommand::RotateCommand() : PIDCommand("Rotate",1,0,0,0.1,0.01) {
 ...
 SmartDashboard::PutString("gyro pid source type", PIDTypeName(RobotMap::ahrs->GetPIDSourceType()));
}
...
bool RotateCommand::IsFinished(){
	return IsTimedOut() || IsCanceled() || std::abs(ReturnPIDInput() - GetSetpoint()) < 5.0;
}
...
double RotateCommand::ReturnPIDInput(){
	...
	//return RobotMap::ahrs->PIDGet();//LOL IMPLEMENTS PIDSource but marks api as private ?????!!!!
}
