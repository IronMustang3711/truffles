...
struct Block {
	void print() {
		...
		if (signature > PIXY_MAX_SIGNATURE) {  // Color-code! (CC)
			...
			for (i = 12, j = 0, flag = false; i >= 0; i -= 3) {  // Assigns value to signature, x, y, width, height, and angle
				d = (signature >> i) & 0x07;
				if (d > 0 && !flag) {
					flag = true;
				}
	...
	uint16_t signature;  // Identification number for your object - set it in the PixyMon
	uint16_t x;  // 0-320
	uint16_t y;  // 0-200
	uint16_t width;
	uint16_t height;
...
private:
	I2C* i2c;
	BlockType blockType;
	bool skipStart;	 // Skips to check for 0xaa55, which is the identifier that tells that Pixy is at the start of new frame
...
public:
	...
	Block blocks[PIXY_MAXIMUM_ARRAYSIZE]; // Array that stores the matched signature objects
};
